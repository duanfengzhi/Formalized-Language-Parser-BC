...
{
    public static void Get_char(ref WordType[] tt, ref int tt_i, ref MyArray[] a, ref int a_i, ref string parseresult, ref Dictionary<string, int> varmap, ref Dictionary<string, string> varmap2, ref Dictionary<string, MyArray> varmap3)
    {
        ...
        if (tt[tt_i + 1].Code.Equals("(") && tt[tt_i + 2].Type.Equals("var") && tt[tt_i + 3].Code.Equals(")") && tt[tt_i + 4].Code.Equals(";"))
        {
            ...
            if (varmap.ContainsKey(tt[tt_i].Code))
            {
                ...
                parseresult += "输入完成：\r\n";
                varmap2[tt[tt_i].Code] = stringInput;
